...
class CleanUpCommand extends Command {
  async run() {
    ...
    if (surgeCustomDomainTemplate.indexOf('#{PR}') < 0) this.error("'surgeCustomDomainTemplate' needs to have '#{PR}' in it");

    const deployedPRs = getDeployedPRs(surgeLogin, surgeToken, surgeCustomDomainTemplate);
